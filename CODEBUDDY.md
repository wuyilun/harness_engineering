...
大型需求按功能模块拆分为多个独立迭代，每个迭代完成完整的 需求→方案→开发→测试→验收 循环。

### 上下文传递增强

PM 派发任务时传递**浓缩上下文**（六要素：任务概述、上游决策、核心上下文摘要、参考文件、约束注意点、交付标准），而非仅传文件路径，减少信息逐层衰减。

### 断链恢复

PM 在每个阶段完成后保存 `project/session_state.json`，断链后从断点续传。
