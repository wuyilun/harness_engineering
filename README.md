...
本仓库是一套基于 [CodeBuddy](https://www.codebuddy.ai) 平台的完整 Harness 实现，包含 **6 个专业 Agent**、**5 条硬约束规则**、**32 个技能包**、[OpenSpec](https://openspec.pro) 规范驱动开发 和 **分层记忆体系**，可直接用于创建高质量的全栈软件项目。
...
| 🏗️ **产品架构专家** | glm-5.1 | 需求分析 + 技术方案设计（一体化） | 只产出需求+方案+API 契约 |
| 🎨 **前端开发专家** | glm-4.7 | 前端代码实现 | 只修改 `src/frontend/` |
| ⚙️ **后端开发专家** | glm-4.7 | 后端代码实现 | 只修改 `src/backend/` |
| 🧪 **QA 测试专家** | glm-4.7 | 前后端测试 + 契约一致性验证 | 只写测试，不修业务代码 |
| ✅ **功能验收专家** | glm-4.7 | 功能验收 | 只读代码，不修改任何文件 |

### 模型分配策略

- **glm-5.1**（强推理）：PM 专家、产品架构专家 — 需要全局推理、权衡决策
- **glm-4.7**（高效执行）：开发、测试、验收专家 — 需要执行、生成、校验

### 为什么合并？

| 合并前（8 Agent） | 合并后（6 Agent） | 合并理由 |
|-----------------|-----------------|---------|
| 需求专家 + 技术方案专家 | **产品架构专家** | 消除需求→方案间最大的信息损失，零上下文切换 |
| 前端测试 + 后端测试 | **QA 测试专家** | 统一测试报告，新增跨端契约一致性验证能力 |
...
|---------|---------|---------|
| 小型 | ≤ 3 个功能点 | 单次循环 |
| 中型 | 4-8 个功能点 | 按模块拆 2-3 个迭代 |
| 大型 | > 8 个功能点 | 按模块拆 3+ 个迭代，每个独立验收 |

每个迭代独立完成 需求 → 方案 → 开发 → 测试 → 验收 的完整循环。

### 执行流程

```
迭代1：功能模块A → 需求 → 方案 → 开发 → 测试 → 验收 ✅
迭代2：功能模块B（依赖A）→ PM 传递迭代1上下文 → 需求 → ... → 验收 ✅
迭代3：功能模块C（依赖A+B）→ PM 传递迭代1+2上下文 → 需求 → ... → 验收 ✅

单次迭代内部：
需求专家 ──(串行)──→ 技术方案专家 ──(并行)──→ 前端+后端开发 ──(并行)──→ 前端+后端测试 ──(串行)──→ 验收
```

### 上下文传递（核心优化）

PM 派发任务时传递**浓缩上下文**，而非仅传文件路径：

| 传统做法（❌） | 优化做法（✅） |
|--------------|-------------|
| 只传文件路径 | 传递任务概述 + 上游决策 + 核心上下文摘要 |
| subagent 自行读文件理解 | PM 先读懂再转述关键信息 |
| 信息逐层衰减 70%→80%→80% | PM 直接传递理解，减少损耗 |

### 断链恢复

PM 在每个阶段完成后保存 `project/session_state.json`，断链后可从断点续传，无需从头开始。

### 流转规则

1. **上游产出未完成，下游不启动**
2. **Agent 之间不直接通信**，所有流转通过 PM 中转
3. **PM 传递浓缩上下文**，包含任务概述、关键决策、核心信息摘要
4. **测试专家发现问题报回 PM**，不直接修改代码
5. **验收不通过报回 PM**，由 PM 决定回退环节
6. **同一问题最多迭代 3 轮**
7. **PM 绝不替代其他专家执行任务**
8. **禁止因等待而自行行动**
...
| OpenSpec 命令 | Harness 阶段 | 执行 Agent | 执行顺序 |
|:--------------|-------------|-----------|---------|
| `/opsx:propose "功能"` | 需求+方案 | 产品架构专家 | **串行**（一体化完成） |
| `/opsx:apply` | 并行开发 | 前端+后端开发 | **并行**（方案完成后） |
| `/opsx:explore` | 需求探索 | 产品架构专家 | 纯思考，不触发后续 |
| `/opsx:archive` | 归档 | 验收专家 | **串行最后** |

### OpenSpec 产物结构

```
openspec/changes/{feature-name}/
├── proposal.md    # 做什么、为什么（产品架构专家产出）
├── specs/         # 需求与验收场景（产品架构专家产出）
│   └── {capability}/spec.md
├── design.md      # 技术方案（产品架构专家产出）
└── tasks.md       # 实现任务清单（产品架构专家产出）
```
...
| Skill | 关联 Agent | 职责 |
|-------|-----------|------|
| `requirement-analysis` | 产品架构专家 | 需求拆解工作流 + NFR 检查清单 |
| `tech-design` | 产品架构专家 | 技术方案工作流 + API 设计规范 + OpenAPI 3.0 |
| `frontend-dev` | 前端开发专家 | 前端开发工作流（集成 Impeccable）+ React/Vue 指南 |
| `backend-dev` | 后端开发专家 | 后端开发工作流（Python/FastAPI 分层架构）+ 初始化脚本 |
| `frontend-testing` | QA 测试专家 | 前端测试 + React Testing Library / Vue Test Utils |
| `backend-testing` | QA 测试专家 | 后端测试 + pytest + API 契约一致性验证 |
...
1. **克隆本仓库**
   ```bash
   git clone git@github.com:wuyilun/harness_engineering.git my-project
   cd my-project
   ```
...
5. **观察自动化流程**
   ...
   ```
   PM → 产品架构专家（产出需求文档 + 技术方案 + API 契约）
      → 前端开发 + 后端开发（并行实现，自动初始化项目）
      → QA 测试专家（前后端测试 + 契约一致性验证）
      → 功能验收专家（逐条验收）
      → 交付
   ```
...
这是职责分离的核心——如果测试专家可以修改代码，就失去了测试的独立性。正确做法是：QA 发现问题 → 报回 PM → PM 派发开发专家修复 → QA 重新验证。
